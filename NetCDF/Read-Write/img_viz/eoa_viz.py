...
class EOAImageVisualizer:
    ...
    def plot_slice_eoa(self, c_img, ax, cmap='gray', mode=PlotMode.RASTER) -> None:
        """
        ...
        if mode == PlotMode.RASTER or mode == PlotMode.MERGED:
            if np.isnan(self._mincbar):
                im = c_ax.imshow(c_img, cmap=cmap)
            else:
                im = c_ax.imshow(c_img, cmap=cmap, vmin=self._mincbar, vmax=self._maxcbar)
        ...
        for c_slice in z_levels:
            for c_time in timesteps:
                ...
                for idx_var, c_var_name in enumerate(var_names):
                    ...
                    cur_var = ds[c_var_name].sel(**{z_levelvar: c_depth, timevar:c_time}, method='nearest')
        ...
        for c_slice in z_levels:
            for c_time in timesteps:
                ...
                for idx_var, c_var_name in enumerate(var_names):
                    ...
                    im = self.plot_slice_eoa(cur_var[c_time, c_slice,:,:], ax, cmap=cmap)
                    c_title = F'{c_var_name} {title} Z-level:{c_slice} Time:{c_time} '
                    ax.set_title(c_title, fontsize=self._font_size)
                    self.add_colorbar(fig, im, ax, show_color_bar)
    ...
    def plot_3d_data_ncds(self, ds, var_names: list, z_levels: list, title='',
        ...
        create_folder(self._output_folder)
        for c_slice in z_levels:
                fig = plt.subplots(1, len(var_names), squeeze=True, figsize=self.get_proper_size(1, len(var_names)))
                for idx_var, c_var_name in enumerate(var_names):
                    cur_var = ds.variables.get(c_var_name)
                    ax = plt.subplot(1, len(var_names), idx_var+1)
    ...
    def plot_3d_data_singlevar_np(self, data:list, z_levels= [], title='',
        ...
        for slice_idx, c_slice in enumerate(z_levels):
            ...
            if flip_data:
                im = self.plot_slice_eoa(np.flip(np.flip(data[c_slice,:,:]),axis=1), ax, cmap=cmap)
            else:
    ...
    def plot_3d_data_np(self, np_variables:list, var_names:list, z_levels= [], title='',
                        ...
                        show_color_bar=True, plot_mode=PlotMode.RASTER):
        """
        ...
        for c_slice in z_levels:
                fig, _axs = plt.subplots(1, len(var_names), squeeze=True, figsize=self.get_proper_size(1, len(var_names)))
                ...
                for idx_var, c_var in enumerate(np_variables):
                    ax = _axs[idx_var]
                    if flip_data:
                        im = self.plot_slice_eoa(np.flip(np.flip(c_var[c_slice, :, :]),axis=1), ax, cmap=cmap, mode=plot_mode)
                    else:
                        im = self.plot_slice_eoa(c_var[c_slice,:,:], ax, cmap=cmap, mode=plot_mode)
    ...
    def plot_2d_data_np(self, np_variables: list, var_names: list, title='',
        ...
        for idx_var, c_var in enumerate(np_variables):
            ax = plt.subplot(1, len(var_names), idx_var+1)
            if flip_data:
                im =self.plot_slice_eoa(np.flip(np.flip(c_var),axis=1), ax, cmap=cmap, mode=plot_mode)
            else:
